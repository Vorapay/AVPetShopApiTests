...
public class TestPet {
    ...
    @Severity(SeverityLevel.CRITICAL)
    @Owner("artemy voropaev")
    public void testGetIdNonexistentPet() {

        Response response = step("Отправить GET запрос на получение ID несуществующего питомца", () ->
                given()
                        .contentType(ContentType.JSON)
                        .header("Accept", "application/json")
                        .when()
    ...
    @ParameterizedTest(name = "Добавление питоица со статусом: {2}")
    @CsvSource({
            "200, Koshka, available",
            "201, Puppy, pending",
            "202, Luppy, sold",
            "203, Pety, unavailable"
    })
    @Feature("Pet")
    @Severity(SeverityLevel.CRITICAL)
    @Owner("artemy voropaev")
